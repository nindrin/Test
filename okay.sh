...
Open_port(){
  ...
  firewall-cmd --zone=public --add-port=${ssr_port}/tcp --permanent  # 开放2333端口
}
Del_port(){
  firewall-cmd --zone=public --add-port=${port}/udp --permanent
  firewall-cmd --zone=public --add-port=${port}/tcp --permanent  # 开放2333端口
}
# 读取 配置信息
Get_IP(){
...
Set_config_port(){
	while true
	do
	echo -e "请输入要设置的ShadowsocksR账号 端口(默认: 5244):"
...
Set_config_password(){
	echo "请输入要设置的ShadowsocksR账号 密码(默认: nindrin):"
	#read -e -p "(默认: doub.io):" ssr_password
	#[[ -z "${ssr_password}" ]] && ssr_password="doub.io"
	ssr_password="nindrin"
...
Set_config_protocol(){
 ...
 (默认: 2. auth_sha1_v4):" && echo
	...
	if [[ ${ssr_protocol} != "origin" ]]; then
		...
		echo -e "是否设置 协议插件兼容原版(_compatible)？[Y/n] (默认 Y)"
...
Set_config_protocol_param(){
	...
	echo -e "${Tip} 设备数限制：每个端口同一时间能链接的客户端数量(多端口模式，每个端口都是独立计算)，建议最少 2个。(默认: 无限):"
...
Set_config_speed_limit_per_con(){
	...
	echo -e "${Tip} 单线程限速：每个端口 单线程的限速上限，多线程即无效。(默认: 无限):"
...
Set_config_speed_limit_per_user(){
	...
	echo -e "${Tip} 端口总限速：每个端口 总速度 限速上限，单个端口整体限速。(默认: 无限):"
...
# 下载 ShadowsocksR
Download_SSR(){
  wget -N --no-check-certificate "https://raw.githubusercontent.com/nindrin/Test/main/main.zip"
  mv "main.zip" "/usr/local/shadowsocksr-manyuser.zip"
	cd "/usr/local/"
	...
	[[ ! -e "shadowsocksr-manyuser.zip" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 未找到 !" && rm -rf manyuser.zip && exit 1
	...
	echo -e "${Info} ShadowsocksR服务端 安装完成 !"
}
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/nindrin/Test/main/centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/nindrin/Test/main/debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
